        ...
        if (typeof(items) !== 'undefined'){
          ...
          if(items !== null){
               ...
               return items.map((row, index)=>{
                ...
                // 
                return <tr key={index}><td>{index+1}</td><td>{row.filename}</td><td>{row.time_uploaded}</td><td> <Button type="submit" variant="warning" size="sm" inline="true" onClick={()=>setModeEdit(row)} size="sm" className="px-1 py-0" ><PencilSquare size={12} /></Button> <Button type="submit" variant="danger" size="sm" onClick={()=>setModeDelete(row)} className="px-1 py-0" ><Trash size={12} /></Button></td></tr>
                })
               }else{
                ...
                <Card.Body>
                    ...
                    <Table bordered className="font-11" size="sm">
                        <thead>
                            <tr>
                                <th width="5%">No</th>
                                <th width="40%">Metadata File</th>
                                <th width="40%">Time Uploaded</th>
